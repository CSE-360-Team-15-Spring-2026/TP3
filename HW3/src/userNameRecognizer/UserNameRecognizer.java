...
public class UserNameRecognizer {
	...
	/** The input string currently being processed by the recognizer. */
	public static String userNameRecognizerInput = "";			// The input being processed
	/** The index within the input where the error was found; -1 if no error. */
	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 0: 
				...
				// The current character is checked against A-Z, a-z, (no 0-9 since first character is alphabetic). If any are matched
				// the FSM goes to state 1
				
				// A-Z, a-z, (no 0-9, since first character is alphabetic)  -> State 1  
				if ((currentChar >= 'A' && currentChar <= 'Z' ) ||		// Check for A-Z
						(currentChar >= 'a' && currentChar <= 'z' )) {	// Check for a-z
					// (no 0-9 since first character is alphabetic)	(currentChar >= '0' && currentChar <= '9' )) {	// Check for 0-9
					nextState = 1;
			...
			case 2: 
				 // State 2 deals with a character after a period, dash, or underscore in the name.				
				// A-Z, a-z, 0-9 -> State 1
				if ((currentChar >= 'A' && currentChar <= 'Z' ) ||		// Check for A-Z
						(currentChar >= 'a' && currentChar <= 'z' ) ||	// Check for a-z
						(currentChar >= '0' && currentChar <= '9' )) {	// Check for 0-9
		...
		case 1:
			...
			if (userNameSize < 4) {
				...
				userNameRecognizerErrorMessage += 
					"A UserName character may only contain the characters A-Z, a-z, 0-9, period, dash, or underscore.\n";				
				return userNameRecognizerErrorMessage;
			}
			else {
					// UserName is valid
